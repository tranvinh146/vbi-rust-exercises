...
fn main() {
    ...
    loop {
        ...
        let check_input: Vec<&str> = trimmed_text.split(" ").collect();
        if check_input[0] == "Add" && check_input[2] == "to" {
            break;
        } else {
...
//                 if let Some(price) = self.price(*item_name) {
//                     total_price += price
//                 } else {
//                     return None;
//                 };
